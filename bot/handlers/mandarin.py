...
# ===== Команда /send_stat - ссылка на статистику =====

@router.message(Command("send_stat"))
async def cmd_send_stat(message: Message):
    """Отправить ссылку на страницу статистики."""
    # Замени на свой домен/IP
    stats_url = "http://89.175.243.106:8000/stats"
    
    await message.answer(
        f"📊 <b>Статистика команд</b>\n\n"
        f"Открой страницу для просмотра всех баллов:\n"
        f"👉 {stats_url}\n\n"
        f"<i>Страница обновляется каждые 10 секунд</i>",
        parse_mode="HTML"
    )
